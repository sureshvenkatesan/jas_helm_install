...
```
The described scenario might happen if Distribution was installed without providing a value to "postgresql.postgresqlPassword" - In this case, a password was autogenerated every time you run the "helm upgrade --install" command and it may cause a password mismatch between PostgreSQL DB and Distribution application.

Since it is a new installation (assuming there is no data at all), we might solve this issue by the followings steps:

a) 1. Uninstall the chart - 
